...
class KnowledgeCompletionSessionTests(unittest.TestCase):
    ...
    def test_prepare_marks_zero_residual_plan_complete(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for row in rows:
                row["coverage"] = "1.0"
                row["fully_semantic"] = "True"
                row["residual_tokens"] = ""
                if not row["instrument_ids"] and not row["concept_ids"]:
                    row["concept_ids"] = "concept:synthetic"
            with csv_path.open("w", newline="", encoding="utf-8-sig") as f:
            ...
            self.assertIn("[knowledge-completion] complete", result.stdout)
            plan = json.loads(
                (
                    out
                    / "reports"
            ...
            self.assertEqual(plan["counts"]["semantic_residual_expressions"], 0)
            self.assertEqual(plan["counts"]["batch_count"], 0)
            self.assertEqual(plan["reports"]["review_batches"], [])
            self.assertIn("Semantic completion achieved", plan["next_action"])
